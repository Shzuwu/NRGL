...
class LabelDivision(nn.Module):
    def __init__(self, args, input_dim, clean_set, noise_set):
    ...
    def division(self, idx_train, emb_lp, emb_hp, label_noise, epoch):
        ...
        num_clean = len(label_clean)
        num_correct = len(label_correct)
        num_remain = len(label_remain)
        
        clean_ratio = torch.sum(self.clean_set[label_clean])/len(label_clean)
        noise_ratio = torch.sum(self.noise_set[label_correct])/len(label_correct)
        
        return index, score, y_train_noise, num_clean, num_correct, num_remain, clean_ratio, noise_ratio
